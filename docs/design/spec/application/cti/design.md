...
- **内部通信**：
  - 与 sipserver/sigserver：通过 Redis Pub/Sub（详见 2.4 通信矩阵）
...
| cti-server | sipserver / sigserver | Redis Pub/Sub | `call:command:{instance_id}` 下发命令，`call:event:{instance_id}` 接收事件 |
...
> **与信令服务器认证方式的差异说明**：
>
> 信令服务器（sigserver）使用**连接前 URL 参数认证**（`wss://{host}/ws?token=<token>&extension=<ext>`），因为信令连接需要立即绑定分机号，建立长生命周期连接。
>
...
> `instance_id` 为 sipserver/sigserver 的实例标识，CTI 根据坐席绑定的信令实例选择对应的 command channel 发送指令。
