...
└────┬─────┘            └────┬─────┘
     ...
     └───────────┬───────────┘
          ...
          └──────┬──────┘
                 │
          ┌──────┴──────┐
          │  medserver  │  媒体桥接 + 呼叫处理（ProcessCall）
          └─────────────┘
```
...
所有路由决策均在信令服务器（sipserver/sigserver）本地执行：

| 层级 | 执行者 | 范围 | 说明 |
|------|--------|------|------|
| 源识别 | sipserver 本地 | 来电来源判定 | 基于 DID、中继、主叫号码识别租户和路由上下文 |
| 规则匹配 | sipserver 本地 | 路由规则评估 | 快速规则匹配 + 图引擎决策，本地缓存 ~50K QPS |
| 呼叫处理 | medserver (gRPC ProcessCall) | 媒体处理 | IVR 导航、ACD 排队、呼叫流程编排、会议、录音 |

信令服务器本地完成两层路由决策（快速规则 + 图引擎），决策完成后通过 gRPC `ProcessCall` 委托 medserver 执行呼叫处理（IVR、队列、图引擎节点等）。
...
├── router       — 本地路由匹配器（两层路由：快速规则 + 图引擎，全部在本地执行）
...
┌──────────────────────────────────────────────────────────────────┐
│  Proxy / Registrar                                                │
│  ├─ 本地路由决策（两层路由：快速规则 + 图引擎，本地缓存）                │
│  ├─ 路由决策后通过 gRPC ProcessCall 委托 medserver                   │
│  ├─ 查询注册表（Redis）定位被叫                                     │
│  ├─ 判断是否需要 B2BUA 降级                                        │
│  └─ 转发或降级处理                                                 │
└──────────────────────────┬───────────────────────────────────────┘
...
sipserver 内部的路由匹配器（`router` 模块）执行完整的两层路由决策（快速规则 + 图引擎），专为 ~50K QPS 的低延迟查询设计。所有路由决策在本地完成，决策后通过 gRPC `ProcessCall` 委托 medserver 执行呼叫处理（IVR、ACD、呼叫流程编排等）。

**纯 Proxy 路径（默认）**：
1. 解析 Request-URI，提取被叫号码/域名
2. 本地路由决策（两层路由：快速规则 + 图引擎，本地缓存）
3. 判断是否需要 B2BUA 降级
4. 查注册表定位被叫
...
**路由决策后的呼叫处理**（通过 gRPC ProcessCall 委托 medserver）：
- 呼叫流程编排（call_flow_id 绑定）
- IVR 导航
- ACD 队列分配
- 时间条件路由

> **设计说明**：路由决策逻辑全部在信令服务器本地执行（含快速规则和图引擎），配置数据模型详见 routing-engine-design。决策完成后通过 gRPC `ProcessCall` 与 medserver 协作完成呼叫处理。
...
│  ④ 路由决策                                                      │
│                                                                  │
│  信令服务器本地路由决策（两层路由：快速规则 + 图引擎）：                     │
│  ├─ DID 直接绑定分机 → 直接路由到目标分机                            │
│  ├─ 路由规则匹配命中 → 按 action 执行                               │
│  ├─ DID 绑定了 call_flow_id → 执行呼叫流程编排                      │
│  ├─ 时间条件路由 → 本地评估时间条件                                  │
│  ├─ IVR 流程 → 本地决策后通过 ProcessCall 委托 medserver            │
│  ├─ ACD 排队 → 本地决策后通知 cti-server 执行队列分配策略            │
│  └─ 无匹配 → 播放"号码不存在"提示 → 挂断                            │
│                                                                  │
...
sipserver 本地路由匹配器按优先级从高到低逐条匹配，首条命中即执行。所有路由规则（含两层路由：快速规则 + 图引擎）均在信令服务器本地执行。

**数据模型**：

```rust
/// 本地路由规则（完整两层路由：快速规则 + 图引擎）
struct RouteRule {
    id: i64,
    tenant_id: i64,
    priority: i32,                          // 越高越优先
...
enum RouteAction {
    ...
    Queue { queue_id: i64 },                // ACD 排队（通知 cti-server）
    Ivrs { ivr_flow_id: i64 },              // IVR 流程（ProcessCall 委托 medserver）
    CallFlow { call_flow_id: i64 },         // 呼叫流程（ProcessCall 委托 medserver）
    Conference { conf_id: i64 },            // 会议（ProcessCall 委托 medserver）
    Deny,                                   // 拒绝
}
```

> **注意**：所有路由动作均在信令服务器本地决策。分机直拨和中继路由可直接在 sipserver 内执行（Proxy 模式）；IVR、队列、呼叫流程、会议等需要通过 gRPC `ProcessCall` 委托 medserver 执行呼叫处理（B2BUA 模式）。
...
输入：(tenant_id, caller, callee, did_id, trunk_id, current_time)
  ...
  │     ├─ 复杂动作（Queue/Ivrs/CallFlow）→ gRPC ProcessCall 委托 medserver
  │     ├─ 成功 → 结束
  │     └─ 失败（如分机未注册）→ 执行 fallback_action
  │           └─ fallback 也失败 → 播放提示音后挂断
  │
  └─ ⑤ 无规则命中 → 播放"号码不存在"提示 → 挂断
```
...
// 规则 2：特定 DID → 呼叫流程编排
{
  ...
  "action": { "type": "CallFlow", "call_flow_id": 5 },
  "time_condition_id": 1,
  "fallback_action": { "type": "CallFlow", "call_flow_id": 6 }
}
```

#### 5.1.2 IVR 执行引擎（信令服务器决策 + medserver 媒体处理）
...
来电路由到队列后的处理流程（信令服务器本地决策后通过 Redis Pub/Sub 协调 cti-server 完成）。
...
| sipserver → medserver | gRPC | ProcessCall 呼叫处理（含 IVR、ACD、呼叫流程编排） |
...
- gRPC（medserver）：通过 gRPC metadata 传递
...
- `trace_id` 用于跨服务关联（sipserver ↔ sigserver ↔ medserver）
...
**状态判定**：
- `healthy`：所有依赖正常
- `degraded`：部分依赖降级但核心功能可用（如 DB 熔断、Redis 走缓存）
...
| 8 | medserver 不可用 | 降级：跳过媒体处理，走 fallback_action |
