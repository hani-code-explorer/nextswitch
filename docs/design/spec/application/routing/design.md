...
> **注意（2026-09-10）**：本文档描述的路由引擎原设计为独立服务（router-server）。该架构已更新：路由决策逻辑现由信令服务器（sipserver/sigserver）本地执行，配置数据模型与规则定义不变，但执行环境从独立 gRPC 服务迁移至信令服务器内部模块。详见 `2026-09-10-routing-config-design.md`。

**版本**: 2.0.0  
**日期**: 2026-09-09  
**状态**: 设计完成（架构已更新，路由逻辑整合至信令服务器）
