...
nextswitch-api, config-service, cti-server-01, im-server-01
```
...
| sipserver | `sipserver-{N}` | sigserver-*, medserver-*, cti-server-* |
| sigserver | `sigserver-{N}` | sipserver-*, medserver-*, cti-server-* |
...
6. ~~**新增 router-server 到信任矩阵**~~（3.5 节）：router-server 已不再作为独立服务，该条目已移除。
