...
> 本文档是 [信令服务器细化设计索引](2026-09-10-signaling-00-index.md) 的第 1 部分。
...
| sipserver | router-server | gRPC | `RoutingService.RouteInteraction()` | Protobuf | 1s | 1 次 | 本地快速路径匹配 |
...
> **端点来源**：所有 gRPC 调用的目标端点地址通过 Redis 服务注册表（`svc:{service}:{instance_id}`）动态获取，由本地 `ServiceDiscoveryCache` 缓存。本地缓存 TTL 15s，后台每 5s 刷新。详见 [配置管理设计](2026-09-10-signaling-06-config-design.md) §6。
...
## 3. 与 router-server 交互

### 3.1 gRPC 调用模式

sipserver 在以下场景调用 router-server 的 `RouteInteraction()` RPC：

| 场景 | 触发条件 | RouteRequest 关键字段 |
|------|---------|---------------------|
| DID 绑定呼叫流程 | DID 的 `call_flow_id` 非 NULL | `route_point_id`, `callee`, `headers` |
| 本地快速路径无匹配 | 本地路由规则全部未命中 | `caller`, `callee`, `trunk_id` |
| 时间条件路由 | 路由规则引用了 `time_condition_id` | `caller`, `callee`, `headers` |
| IVR 流程入口 | 路由点 `route_type=ivr` | `callee`, `route_point_id` |
| ACD 排队 | 路由点 `route_type=queue` | `caller`, `callee`, `route_point_id` |

```rust
/// sipserver 调用 router-server 的完整流程
pub async fn route_interaction(
    ...
    let request = RouteRequest {
        ...
        interaction_id: ctx.call_id.clone(),
        interaction_type: InteractionType::Voice.into(),
        caller: ctx.caller.clone(),
        callee: ctx.callee.clone(),
        headers: ctx.sip_headers.clone(),
        variables: ctx.variables.clone(),
        route_point_id: ctx.route_point_id.unwrap_or(0),
    };
    
    // gRPC 调用（带超时和重试）
    let result = tokio::time::timeout(
        Duration::from_millis(1000),
        self.router_client.route_interaction(request.clone()),
    ).await;
    ...
    match result {
        ...
        Ok(Err(status)) => {
            // gRPC 调用失败，降级到本地路由
            metrics::counter!("grpc_client_errors_total",
                "service" => "router-server",
                "method" => "route_interaction",
                "code" => status.code().to_string(),
            ).increment(1);
            self.fallback_to_local_routing(ctx).await
        }
        Err(_timeout) => {
            // 超时，降级到本地路由
            metrics::counter!("grpc_client_errors_total",
...
当 router-server 不可达时，sipserver 降级到本地快速路径匹配：

```rust
impl SipServer {
    /// 降级路由：仅支持本地快速路径
    async fn fallback_to_local_routing(
        &self,
        ctx: &CallContext,
    ) -> Result<RouteResponse, RoutingError> {
            ...
            match rp.route_type {
                RouteType::Extension => {
                    return Ok(RouteResponse {
                        decision: RouteDecision::Direct.into(),
                        target: Some(RouteTarget {
                            target: Some(target::Target::Direct(DirectTarget {
                                extension: rp.route_target.clone(),
                                contact_uri: String::new(), // 需查注册表
                            })),
                        }),
                        ..Default::default()
                    });
                ...
                RouteType::Trunk => {
                    return Ok(RouteResponse {
                        decision: RouteDecision::Trunk.into(),
                        target: Some(RouteTarget {
                            target: Some(target::Target::Trunk(TrunkTarget {
                                trunk_group: rp.route_target.clone(),
                ...
                // 复杂路由类型降级失败
                _ => return Err(RoutingError::FallbackNotSupported),
            }
        }
        
        // Step 3: 规则匹配（本地 routing_rules）
...
### 3.3 RouteResponse 处理

router-server 返回的 `RouteDecision` 决定 sipserver 的后续行为：

| RouteDecision | sipserver 动作 |
|---------------|---------------|
| `DIRECT` | 查注册表，转发 INVITE 到目标分机 |
| `QUEUE` | 通过 Redis Pub/Sub 通知 cti-server 执行 ACD 排队 |
| `IVR` | B2BUA 降级，请求 medserver 创建 IVR 媒体会话 |
| `TRUNK` | 转发 INVITE 到目标中继 |
| `CALL_FLOW` | B2BUA 降级，启动图引擎流程实例 |
| `CONFERENCE` | B2BUA 降级，请求 medserver 创建会议会话 |
| `EXTERNAL` | 转发到外部路由目标 |
...
> 详见 [medserver 接口设计](2026-09-10-signaling-04-medserver-interface.md)
...
当 router-server 返回 `QUEUE` 决策时，sipserver 与 cti-server 的协作流程：
...
> 完整健康检查规范详见 [监控设计](2026-09-10-signaling-05-monitoring-design.md) §3。
