...
> 本文档是 [信令服务器细化设计索引](2026-09-10-signaling-00-index.md) 的第 5 部分。
...
│   │   ├── span: sip.local_route_match (本地匹配)
│   │   └── span: grpc.router_server (远程路由) [如需要]
...
| `SipRouterServerDown` | `grpc_client_errors_total{service="router-server"} > 5` 在 1m 内 | P2 | 通知运维（降级到本地路由） |
...
│                                                                     │
│  ┌─────────────────────────────┐ ┌─────────────────────────────┐   │
│  │ Redis 操作延迟/命中率         │ │ gRPC 调用延迟（router/med）  │   │
│  │ [双轴折线图]                 │ │ [折线图]                      │   │
│  └─────────────────────────────┘ └─────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────┘
```
